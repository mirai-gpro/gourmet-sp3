...
**変更しないもの**: `_handle_tool_call()`, `_handle_shop_search()`, `_describe_shops_via_live()`, `_receive_shop_description()`, 再接続関連コード全般
...
**変更なし。** `shop_search_result` イベントハンドラは既に `data.shops` でカード表示、`data.response` でチャットテキスト表示を行っており、そのまま維持。
...
- [ ] グルメモード: 条件指定 → 即座にsearch_shops発火 → カード表示 → 読み上げ → 通常会話復帰
- [ ] コンシェルジュモード: ヒアリング → 条件確定 → search_shops発火 → カード表示 → 読み上げ → 通常会話復帰
- [ ] 再検索: 「もっと安いところ」等 → 再度search_shops発火 → 新しいカード表示
...
- コード変更の実態は `LIVEAPI_SHOP_CARD_RULES` プロンプトの書き換えのみ
...
```
変更ファイル: 1ファイル（live_api_handler.py）
変更箇所: プロンプト定義部分のみ（LIVEAPI_SHOP_CARD_RULES → LIVEAPI_SHOP_SEARCH_RULES）
変更行数: 約60行の書き換え（L42-107のプロンプト + L126,220,244-249の変数名参照）
```
