...
│                                                              │
│  入力: LPCM 16kHz float32 (base64) ※MP3不可                  │
...
async def _send_to_a2e(self, pcm_bytes: bytes, chunk_index: int):
    ...
    """
    try:
        # 24kHz 16bit mono → 16kHz float32 へリサンプリング
        audio_16k = prepare_audio_for_a2e(pcm_bytes)
        audio_b64 = base64.b64encode(audio_16k.astype(np.float32).tobytes()).decode('utf-8')

        # A2Eサービスに送信（16kHz float32 LPCM）
        import aiohttp
        async with aiohttp.ClientSession() as http:
            async with http.post(
                ...
                json={
                    ...
                    "audio_format": "pcm_16000_float32",
...
> **⚠️ MP3変換禁止：**
> A2E（LAM）内部の音声特徴抽出器（HuBERT等）は **16kHz LPCM** を前提に学習されている。
> MP3の圧縮ノイズはリップシンクの滑らかさを損なう原因になるため、MP3変換は行わない。
> またMP3エンコードには「フレームの蓄積」が必要なため数十msの遅延が発生するが、
> LPCMのリサンプリングは1サンプル単位で処理できるため遅延をほぼゼロに抑えられる。

**音声データフロー仕様（LPCM直接処理）：**

| ステップ | データ形式 | サンプリングレート | 役割 |
|---------|-----------|----------------|------|
| 1. LiveAPI出力 | LPCM (Base64) | 24,000 Hz | Geminiの生の声 |
| 2. 分岐（並列） | LPCM (Binary) | 24,000 Hz | ブラウザ再生とA2E送信用に分離 |
| 3. リサンプリング | LPCM (Binary) | 16,000 Hz | A2E（LAM）の要求仕様に適合 |
| 4. A2E入力 | LPCM (float32 array) | 16,000 Hz | 表情パラメータの生成 |

`_send_to_a2e()` 内でバックエンド（Python）がリサンプリングを実施する：

```python
import numpy as np
from scipy import signal

def prepare_audio_for_a2e(pcm_24k_data: bytes) -> np.ndarray:
    """
    LiveAPI出力（24kHz 16bit mono LPCM）をA2E入力用（16kHz float32）に変換

    ✅ MP3変換は行わない（精度低下・遅延増加の原因）
    ✅ resample_poly は高速でストリーミング向き（24:16 = 3:2）
    """
    # バイト列 → numpy配列(float32) に変換
    audio_fp32 = np.frombuffer(pcm_24k_data, dtype=np.int16).astype(np.float32) / 32768.0

    # 24kHz → 16kHz へダウンサンプリング（比率 2:3）
    audio_16k = signal.resample_poly(audio_fp32, 2, 3)

    return audio_16k  # これをそのままA2Eモデルへ渡す


# audio2exp-service/app.py 側の受け口
# audio_format は "pcm_16000_float32" で統一する
if audio_format == "pcm_16000_float32":
    # バックエンドで16kHzに変換済みのfloat32配列をそのまま推論に渡す
...
> **⚠️ MP3変換禁止：** LiveAPIのPCMをMP3にパックしてA2E側でデコードし直す処理は
> 精度低下・レイテンシ増加・計算リソースの無駄の三重苦になる。行わないこと。

`audio2exp-service` には **16kHz float32 LPCM**（バックエンドでリサンプリング済み）を渡す。
`audio_format: "pcm_16000_float32"` を受け付けるよう実装する（MP3パスは廃止）。
...
2. LPCM入力対応を追加（バックエンドで24→16kHzリサンプリング後、`audio_format: "pcm_16000_float32"` で受け渡し）
...
### 11.1 バックエンド
- [ ] audio2exp-service がLPCM入力（16kHz float32）を受け付ける
