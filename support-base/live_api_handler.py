...
3. **同意を得たら search_shops ツールを呼び出す**
   ...
   **重要: ステップ1でユーザーに提示した条件要約をそのまま user_request に使うこと。**
   条件を再生成・省略・言い換えしてはならない。確認済みの条件文 = 検索条件。
...
- user_request にはステップ1で提示した確認済み条件をそのまま渡す
    ...
    async def _handle_shop_search(self, user_request: str):
        ...
        try:
            ...
            if not shop_data or not shop_data.get('shops'):
                logger.info(f"[ShopSearch] ショップ見つからず: user_request='{user_request}'")
                self.socketio.emit('shop_search_result', {
