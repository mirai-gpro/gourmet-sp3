...
"""
                ...
                try:
                    async with self.client.aio.live.connect(
                        ...
                        if self.session_count == 1:
                            ...
                            logger.info(f"[LiveAPI] 再接続: 履歴{len(self.conversation_history)}件再送 + トリガー '{resume_text[:50]}'")
    ...
    def _get_context_summary(self) -> str:
        ...
        【設計方針（v3改訂: REST版準拠）】
        キーワード抽出は行わない。REST版と同様に：
        1. 会話履歴の再送（send_client_content turns）でGeminiが文脈から把握
        2. プロンプトの短期記憶ルールで「聞き直し禁止」を強く指示
        ここでは最後のAIの質問のみ補足情報として返す。
        """
        if not self.conversation_history:
            return ""

        # 最後のAI発言が質問なら強調（回答待ちであることを明示）
        last_ai = None
        for h in reversed(self.conversation_history):
            if h['role'] == 'ai':
                last_ai = h['text']
    ...
    async def _send_history_on_reconnect(self, session):
        ...
        これにより、Geminiは再接続後も会話文脈を把握し、
        プロンプトの短期記憶ルールと合わせて同じ質問を繰り返さない。
